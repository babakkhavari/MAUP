...
def calibration(specs_path, csv_path, specs_path_calib, calibrated_csv_path):
    ...
    #pop_modelled = 12122999
    #urban_modelled = 0.486
    ...
    #elec_modelled = 0.419
    #rural_elec_ratio = 0.182
    #urban_elec_ratio = 0.672
...
def scenario(specs_path, calibrated_csv_path, results_folder, summary_folder, name):
    """
    ...
    for scenario in scenarios:
        ...
        grid_loss_index = scenario_info.iloc[scenario]["GridLosses"]
        grid_capacity_investment_cost_index = scenario_info.iloc[scenario]["GridCapacityInvestmentCost"]
        disc_index = scenario_info.iloc[scenario]["DiscountRate"]
        ...
        grid_loss = scenario_parameters.iloc[grid_loss_index]['GridLosses']
        grid_capacity_investment_cost = scenario_parameters.iloc[grid_capacity_investment_cost_index]['GridCapacityInvestmentCost']
        discount_rate = scenario_parameters.iloc[grid_capacity_investment_cost_index]['DiscountRate']

        settlements_in_csv = calibrated_csv_path
        settlements_out_csv = os.path.join(results_folder,
                                           '{}-{}_{}_{}_{}_{}_{}_{}.csv'.format(name, tier_index, grid_index, pv_index,  dist_costs, grid_loss_index, grid_capacity_investment_cost_index, disc_index))

        summary_csv = os.path.join(summary_folder,
                                   '{}-{}_{}_{}_{}_{}_{}_{}_summary.csv'.format(name[:-4], tier_index, grid_index, pv_index,  dist_costs, grid_loss_index, grid_capacity_investment_cost_index, disc_index))
        ...
        grid_calc = Technology(om_of_td_lines=0.02,
                               distribution_losses=float(grid_loss),
                               connection_cost_per_hh=125,
                               base_to_peak_load_ratio=0.8,
                               capacity_factor=1,
                               tech_life=30,
                               grid_capacity_investment=float(grid_capacity_investment_cost),
                               grid_penalty_ratio=1,
                               grid_price=grid_price)
        ...
        elements = ["1.Population", "2.New_Connections", "3.Capacity", "4.Investment", "5.LCOE"]
