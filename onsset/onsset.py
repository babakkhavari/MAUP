...
SET_RESIDENTIAL_TIER = 'ResidentialDemandTierCustom'
...
class SettlementProcessor:
    ...
    def classify_substation_distance(substation_distance):
        ...
        """
        # Define bins
        substation_distance_bins = [-5, 0.5, 1, 5, 10, float("inf")]
        # Define classifiers
    ...
    def get_wind_cf(wind_velocity):
        ...
        p_rated = 600
        z = 55  # hub height
        zr = 50  # velocity measurement height
    ...
    def set_residential_demand(self, rural_tier, urban_tier, num_people_per_hh_rural,
        ...
        if max(self.df[SET_CAPITA_DEMAND]) == 0:
            ...
            wb_tier_rural = rural_tier
            wb_tier_urban_clusters = rural_tier
            wb_tier_urban_centers = urban_tier

            # if wb_tier_urban_centers == 6:
            #     wb_tier_urban_centers = 'CustomLow'
            # if wb_tier_urban_clusters == 6:
            #     wb_tier_urban_clusters = 'CustomLow'
            # if wb_tier_rural == 6:
            #     wb_tier_rural = 'CustomLow'
            #
            ...
            #     wb_tier_urban_centers = 'CustomHigh'
            # if wb_tier_urban_clusters == 8:
            #     wb_tier_urban_clusters = 'CustomHigh'
            # if wb_tier_rural == 8:
            #     wb_tier_rural = 'CustomHigh'
            ...
            self.df.loc[self.df[SET_URBAN] == 0, SET_CAPITA_DEMAND] = (self.df[
                 SET_RESIDENTIAL_TIER]) * wb_tier_rural
            self.df.loc[self.df[SET_URBAN] == 1, SET_CAPITA_DEMAND] = (self.df[
                 SET_RESIDENTIAL_TIER]) * wb_tier_urban_clusters
            self.df.loc[self.df[SET_URBAN] == 2, SET_CAPITA_DEMAND] = (self.df[
                 SET_RESIDENTIAL_TIER]) * wb_tier_urban_centers
    ...
    def set_scenario_variables(self, year, num_people_per_hh_rural, num_people_per_hh_urban, time_step, start_year,
        ...
        self.df.loc[self.df[SET_ENERGY_PER_CELL + "{}".format(year)] == 0, SET_ENERGY_PER_CELL + "{}".format(year)] = 0.00001
    ...
    def calc_summaries(self, df_summary, sumtechs, year):
        ...
        df1 = self.df.loc[(self.df[SET_ELEC_FINAL_CODE + "{}".format(year)] == 1) &
                          (self.df[SET_LIMIT + "{}".format(year)] == 1)]

        df3 = self.df.loc[(self.df[SET_ELEC_FINAL_CODE + "{}".format(year)] == 3) &
                          (self.df[SET_LIMIT + "{}".format(year)] == 1)]

        df5 = self.df.loc[(self.df[SET_ELEC_FINAL_CODE + "{}".format(year)] == 5) &
                          (self.df[SET_LIMIT + "{}".format(year)] == 1)]

        df6 = self.df.loc[(self.df[SET_ELEC_FINAL_CODE + "{}".format(year)] == 6) &
                          (self.df[SET_LIMIT + "{}".format(year)] == 1)]

        df7 = self.df.loc[(self.df[SET_ELEC_FINAL_CODE + "{}".format(year)] == 7) &
                          (self.df[SET_LIMIT + "{}".format(year)] == 1)]

        df8 = self.df.loc[(self.df[SET_ELEC_FINAL_CODE + "{}".format(year)] <= 8) &
                          (self.df[SET_LIMIT + "{}".format(year)] == 1)]

        df_summary[year][sumtechs[33]] = 0
        df_summary[year][sumtechs[35]] = 0

        if df_summary[year][sumtechs[8]] ==0:
            df_summary[year][sumtechs[32]] = 0
        else:
            df_summary[year][sumtechs[32]] = sum((df1[SET_NEW_CONNECTIONS + "{}".format(year)] * df1[
                SET_LIMIT + "{}".format(year)] * df1[SET_MIN_OVERALL_LCOE + "{}".format(year)])) / sum(
                (df1[SET_NEW_CONNECTIONS + "{}".format(year)] * df1[SET_LIMIT + "{}".format(year)]))

        if df_summary[year][sumtechs[10]] == 0:
            df_summary[year][sumtechs[34]] = 0
        else:
            df_summary[year][sumtechs[34]] = sum((df3[SET_NEW_CONNECTIONS + "{}".format(year)] * df3[
                SET_LIMIT + "{}".format(year)] * df3[SET_MIN_OVERALL_LCOE + "{}".format(year)])) / sum(
                (df3[SET_NEW_CONNECTIONS + "{}".format(year)] * df3[SET_LIMIT + "{}".format(year)]))

        if df_summary[year][sumtechs[12]] == 0:
            df_summary[year][sumtechs[36]] = 0
        else:
            df_summary[year][sumtechs[36]] = sum((df5[SET_NEW_CONNECTIONS + "{}".format(year)] * df5[
            SET_LIMIT + "{}".format(year)] * df5[SET_MIN_OVERALL_LCOE + "{}".format(year)])) / sum(
            (df5[SET_NEW_CONNECTIONS + "{}".format(year)] * df5[SET_LIMIT + "{}".format(year)]))

        if df_summary[year][sumtechs[13]] == 0:
            df_summary[year][sumtechs[37]] = 0
        else:
            df_summary[year][sumtechs[37]] = sum((df6[SET_NEW_CONNECTIONS + "{}".format(year)] * df6[
            SET_LIMIT + "{}".format(year)] * df6[SET_MIN_OVERALL_LCOE + "{}".format(year)])) / sum(
            (df6[SET_NEW_CONNECTIONS + "{}".format(year)] * df6[SET_LIMIT + "{}".format(year)]))

        if df_summary[year][sumtechs[14]] == 0:
            df_summary[year][sumtechs[38]] = 0
        else:
            df_summary[year][sumtechs[38]] = sum((df7[SET_NEW_CONNECTIONS + "{}".format(year)] * df7[
            SET_LIMIT + "{}".format(year)] * df7[SET_MIN_OVERALL_LCOE + "{}".format(year)])) / sum(
            (df7[SET_NEW_CONNECTIONS + "{}".format(year)] * df7[SET_LIMIT + "{}".format(year)]))

        if len(df8.index) == 0:
            df_summary[year][sumtechs[39]] = 0
        else:
            df_summary[year][sumtechs[39]] = sum((df8[SET_NEW_CONNECTIONS + "{}".format(year)] * df8[
            SET_LIMIT + "{}".format(year)] * df8[SET_MIN_OVERALL_LCOE + "{}".format(year)])) / sum(
            (df8[SET_NEW_CONNECTIONS + "{}".format(year)] * df8[SET_LIMIT + "{}".format(year)]))
